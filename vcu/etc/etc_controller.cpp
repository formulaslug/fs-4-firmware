...
void ETCController::update_state() {
    ...
    state.APPS1_position = clamp(
        (state.APPS1_voltage - APPS1_MIN_VOLTAGE - APPS_BUFFER_VOLTAGE) / (APPS1_MAX_VOLTAGE - APPS1_MIN_VOLTAGE)
    );
    state.APPS2_position = clamp(
        (state.APPS2_voltage - APPS2_MIN_VOLTAGE - APPS_BUFFER_VOLTAGE) / (APPS2_MAX_VOLTAGE - APPS2_MIN_VOLTAGE)
    );
    state.BPPS_position =
