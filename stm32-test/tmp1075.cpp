...
TMP1075::TMP1075(I2C& _wire, const uint8_t _i2cAddress) : wire(_wire), i2cAddress(_i2cAddress), configRegister(0x00FF) {}
...
void TMP1075::writeRegister(const uint8_t pointerRegister, const uint16_t value) {
    // this->wire.start();
    // this->wire.write_byte(this->i2cAddress << 1);
    // this->wire.write_byte(pointerRegister);
    // this->wire.write_byte((value >> 8) & 0xFF);
    // this->wire.write_byte((value >> 0) & 0xFF);
    // this->wire.stop();
    const char buf[] = {
        pointerRegister,
    ...
    this->wire.write(this->i2cAddress << 1, buf, sizeof(buf), false);
}
...
uint16_t TMP1075::readRegister(const uint8_t pointerRegister) {
    // this->wire.start();
    // this->wire.write_byte(this->i2cAddress << 1 | 1);
    // this->wire.write_byte(pointerRegister);
    // // this->wire.endTransmission(false);
    // // this->wire.requestFrom(this->i2cAddress, (uint8_t)2, (uint8_t)true);
    // uint16_t data = this->wire.read_byte(true) << 8 | this->wire.read_byte(false);
    // this->wire.stop();
    uint8_t data[2];
    this->wire.write(this->i2cAddress << 1, (char*)&pointerRegister, 1, true);
    this->wire.read(this->i2cAddress << 1 | 1, (char*)data, sizeof(data), true);
    this->wire.stop();
...
TMP1075::ConversionTime TMP1075::getConversionTime() const {
...
TMP1075::ConsecutiveFaults TMP1075::getFaultsUntilAlert() const {
