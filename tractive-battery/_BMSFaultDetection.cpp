...
/*
        move structs and telemetry data to seperate can class - call can class every so often using
   data from BMS instance move BMSfaultDetection functions to BMS class so we dont have to keep
   passing a reference (in the event queue we would make controller a public function and call that
   ) make changes to precharge ... soon - note ideally precharge runs once per startup we do NOT
   close the precharge relay while the car is running (add real tmp1075 addresses)




        note for precharge - precharge should be run once during startup and once after each time
...
void chargingActions(BMS& BMSInstance) {
    ...
    // we still need to do safety checks on this --- perhaps we can store the data from this in
    // BMSInstance and then have checks in checkForFaults
}
...
void readTemps(BMS& BMSInstance) {
    ...
    for (uint8_t i = 0; i < NUM_BATTERY_MODULES; i++) {
        for (uint8_t j = 0; j < NUM_TEMP_SENSORS_PER_MODULE; j++) {
            BMSInstance.cellTemps[i][j] =
                BMSInstance.chips[i].readTemperatureTMP1075(&BMSInstance.sensors[i][j]);
        }
    }
    ...
    for (uint8_t i = 0; i < NUM_TRAY_TEMP_SENSORS; i++) {
        BMSInstance.trayTempSensors[i].start_conversion(
            true
        ); // assume no e meter here CHANGE LATER
        ThisThread::sleep_for(3ms);
...
void decideBalancing(BMS& BMSInstance) {
    ...
    if (BMSInstance.currentState != BMSInstance.FAULT) {
        ...
        if (BMSInstance.maxVoltage >= BALANCING_THRESHOLD
            && BMSInstance.minVoltage > MIN_CELL_VOLTAGE)
        {
            for (uint8_t i = 0; i < NUM_BATTERY_MODULES; i++) {
...
void readPackCurrent(BMS& BMSInstance) {
    ...
    printf(
        "Current sense Vout: %.3f V  =>  Pack current: %.2f A\n", vout, BMSInstance.packCurrentAmps
    );
}
...
void checkForFaults(BMS& BMSInstance) {
    ...
    // uint8_t voltage_fault_index = 0; // voltage group (1 of 6 parrallel groups) where a fault was
    // detected
    ...
    //  for testing purposes i am going to use the cell temperature limits here, will be updated
    //  later
...
void checkShutdownCircuit(BMS& BMSInstance) {
    if (BMSInstance.Shutdown_In_3V3_Filtered.read() == 0
        || BMSInstance.Shutdown_Out_3V3_Filtered.read() == 0)
    {
