...
BMS::BMS(CAN& CAN_POWERTRAIN, bool charging, Mutex& mainMutex)
    ...
    readCellVoltages();
    for (uint8_t i = 0; i < NUM_TEMP_SENSORS_PER_MODULE; i++) {
        readTemps();
    }
    checkForFaults();
}
...
void BMS::readTemps() {
    ...
    static uint8_t current_temp_sensor = 0;
    
    uint8_t commData[6] = {0};

    uint8_t temp_sense_address = tempSensors[0][current_temp_sensor].i2c_address;
    uint8_t temp_sense_reg = tempSensors[0][current_temp_sensor].temp_reg;
    ...
    for (int i = 0; i < NUM_BATTERY_MODULES; i++) {
        ...
        // Each LSB = 0.0625°C
        temps[i][current_temp_sensor] = ((float)rawTemp) * 0.0625f;
    }

    current_temp_sensor++;
    current_temp_sensor %= NUM_TEMP_SENSORS_PER_MODULE;
