...
bool glvOk() {
    ...
    return (glvVoltage > 11.0f && glvVoltage < 15.0f); //place holder for 11 to 15 volts? we should find out the limit  
}
...
bool preChargeAllowed() {
    return shutdownClosed() && imdOk() && !isBmsFaultActive() && glvOk() && dcBusVoltage < 0.9f * packVoltage; 
}
